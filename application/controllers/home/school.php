...
class School extends Admin_Controller
{
	...
	public function lists()
	{
        ...
        $where = array('sid'=>$sid);
        $count = $this->school_lists->get_count($where);
        ...
        $wherelist['where'] = array('ls.sid'=>$sid);

        $list = $this->school_lists->get_list_by_join($wherelist);
        $data['list'] = $list;
	...
	public function listsadd()
	{
		if(!empty($_POST)){
			...
			if($this->school_lists->add($add)){
				redirect('/home/school/lists?id='.$sid);
			}else{
				exit('error');
			}
