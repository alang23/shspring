...
class Consignment_mdl extends Spring_Model
{
	...
	public function get_list_join($config = array())
	{
		...
		$list = array();
		$list = $this->db->select(
			'co.id,co.suk,co.pro_name,co.price,co.img,co.uid,co.reviewer,co.createtime,co.status,co.tags,
	...
	public function get_one_by_join($config = array())
	{
		...
		$row = array();
		$row = $this->db->select('co.id,co.suk,co.pro_name,co.price,co.tel,co.img,co.uid,co.reviewer,co.createtime,co.status,co.tags,co.reasons,m.username as mname,a.username')
					->from($this->table_name.' as co')
					->join($this->table_member.' as m','m.id=co.uid','left')
					->join($this->table_admin.' as a','a.id=co.reviewer','left')
					->get()->row_array();
